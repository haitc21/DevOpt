...
- Jenkinsfile sử dụng ngôn ngữ **Groovy** 
            ...
            post {
                success {
                    script {
                        sh(
                            script: '''
                            echo "Successfully! Cleaning up Docker images with the same name but different tags..."
                            docker images --format "{{.Repository}}:{{.Tag}}" | grep "^${DOCKER_IMAGE_NAME}:" | grep -v ":${DOCKER_TAG}$" | xargs -r docker rmi -f
                            ''',
                            label: 'Cleaning up Docker images'
                        )
                    }
                }
            }
